...
namespace CRUDGenerator
{
    ...
    {
        private static ResultPage GenerateEditPage(Type type, string className)
        {
            ...
            sb.AppendLine($"	int.TryParse(Session[\"{objectsVar}Id\"], out int id);");
            sb.AppendLine($"	var {objectsVar} = context.{className}s.SingleOrDefault(x => x.Id == id);");
            sb.AppendLine($"	if ({objectsVar} == null) throw new NullReferenceException(\"The specified {className} could not be found.\");");
            sb.AppendLine(string.Empty);
            foreach (var property in type.GetProperties())
        ...
        /// <summary>
        /// Generates markup virtual identical to that generated by the Create generator
        /// since both use the same form fields to modify the same object properties.
        /// </summary>
        /// <param name="type"></param>
        /// <param name="className"></param>
        /// <returns></returns>
        private static ResultPage GenerateEditMarkup(Type type,string className)
        {
            var objectsVar = className.LowerFirstChar();
            var codeFile = new ResultPage
            {
            ...
            sb.AppendLine($"                     </a>Updating a {className}");
            sb.AppendLine("					</div>");
